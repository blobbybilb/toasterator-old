...
type query<T> = { [K in keyof T as string extends K ? never : K]: T[K] };
...
export class DBInterface {
  ...
  openKV(table: string): KVTable {
    return new KVTable(this.openTable<KVData>(table));
  }
...
class DBTable<T extends Item> {
  ...
  createTable(item: T) {
    ...
    const rowNamesString = Object.keys(item)
  ...
  add(item: T) {
    ...
    this.db
      .query(
        `INSERT INTO ${this.table} (${rowNamesString})
                       VALUES (${rowValuesString})`,
      )
      .run();
  }
  ...
  get(
    query:
      | { [P in keyof query<T>]?: ((arg: query<T>[P]) => boolean) | T[P] }
      | Item,
  ): T[] {
    let queryResult: T[];
    ...
    if (
      ...
      queryResult = this.db
        .query(
          `SELECT *
                                         FROM ${this.table}
                                         WHERE (${queryKeysString}) = (${queryValuesString})`,
        )
        .all() as T[];
    }
...
class KVTable {
  t: DBTable<KVData>;
  constructor(t: DBTable<KVData>) {
    this.t = t;
    this.t.createTable({ key: "", value: "" });
  }

  get(key: string): string {
    const res = this.t.get({ key });
...
// const db = new DBInterface(":memory:");
// const kv = db.openKV("test");
// kv.set("hi", "hello");
// console.log(kv.get("hi"));
// kv.set("hi", "bye");
// console.log(kv.get("hi"));
// kv.del("hi");
// console.log(kv.get("hi"));
