...
class DBTable<T extends Item> {
    ...
    add(item: T) {
        ...
        this.db.query(`INSERT INTO ${this.table} (${rowNamesString})
                       VALUES (${rowValuesString})`).run()
    }
    ...
    get(query: { [P in keyof query<T>]?: ((arg: query<T>[P]) => boolean) | T[P] } | Item): T[] {
        ...
        if ((Object.values(query).filter((value) => typeof value !== "function") as validTypes[]).length === 0) {
            ...
            queryResult = this.db.query(`SELECT *
                                         FROM ${this.table}
                                         WHERE (${queryKeysString}) = (${queryValuesString})`).all() as T[]
        }
    ...
    getter(query: { [P in keyof query<T>]?: ((arg: query<T>[P]) => boolean) | T[P] } | Item): () => T[] {
    ...
    del(item: Item) {
        ...
        this.db.query(`DELETE
                       FROM ${this.table}
                       WHERE ${queryKeysString} = (${queryValuesString})`).run()
    }
